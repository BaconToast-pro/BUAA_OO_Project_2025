...
import expr.vari.call.Ord;
import expr.vari.call.Recur;
...
public class Parser {
    ...
    public Factor parseFactor() {
        if (lexer.peek().equals("x") || lexer.peek().equals("y") ||
            ...
            lexer.peek().equals("f") ||
            lexer.peek().equals("g") || lexer.peek().equals("h")) { // Vari
    ...
    public Vari parseVari() {
        if (lexer.peek().equals("x") || lexer.peek().equals("y")) { // Power
            ...
            final String name = lexer.peek(); // name
            lexer.next(); // "("
            lexer.next(); // Factor
            Factor factor1 = parseFactor(); // "," or ")"
            if (lexer.peek().equals(",")) {
                ...
                Factor factor2 = parseFactor(); // ")"
                lexer.next(); // endOfOrd
                return new Ord(name, factor1, factor2);
            }
            lexer.next(); // endOfOrd
            return new Ord(name, factor1);
        } else {
